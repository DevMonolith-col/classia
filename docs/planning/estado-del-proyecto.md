...
- **Quedan 3 diferencias entre `schema.prisma` y la base** (detectadas el 2026-07-26 con `prisma migrate diff`; `prisma migrate status` dice "up to date" porque las 39 migraciones están aplicadas — la deriva es del schema respecto al historial, y por eso no la ve). Ninguna afecta a `verify:rls`, y **no se tocaron** porque dos cambian semántica de borrado, que es una decisión aparte: (a) `auth_sessions_ticketId_fkey` y (b) `access_sessions_approvedById_fkey` — el schema declara relaciones opcionales, cuyo default en Prisma es `ON DELETE SET NULL`, y la base tiene otra acción referencial; alinearlas cambia qué pasa con las sesiones cuando se borra un ticket o un usuario. (c) La base tiene un índice `support_tickets_assigneeId_idx` que el schema no declara; alinear en la dirección del schema lo **borraría**, y hay que decidir si se quiere conservar (probablemente sí, y entonces la corrección va en el schema, no en la base).
