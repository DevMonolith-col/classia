...
// el fallback: no hay nada que resolver todavía.
...
export class FilesDataScopeGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const requiredScope = key ? await this.resolveScope(key) : AccessScope.DATOS_PERSONALES
  ...
  private async resolveScope(key: string): Promise<AccessScope> {
    ...
    // Clave no encontrada en ninguna tabla dueña: no concluyente, fallback conservador.
