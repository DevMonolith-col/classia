...
export default function CertificadosAlumnoPage() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-lg mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-bold flex items-center gap-2"><FileCheck2 className="h-5 w-5 text-primary" /> Mis certificados</h1>
        <p className="text-sm text-muted-foreground">Constancias y boletines emitidos por el colegio.</p>
      </div>

      {loading ? (
        <p className="text-sm text-muted-foreground">Cargando...</p>
      ) : issuances.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-sm text-muted-foreground">Todavía no tenés certificados emitidos.</CardContent></Card>
      ) : (
            ...
            <Card key={doc.id}>
              <CardContent className="flex items-center justify-between gap-3 p-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{new Date(doc.issuedAt).toLocaleDateString("es-CO")}</p>
